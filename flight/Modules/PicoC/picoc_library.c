...
{
	if (Param[0]->Val->UnsignedCharacter < PIOS_GPIO_NUM)
	{
		switch(Param[1]->Val->UnsignedCharacter) {
			case 0:
				PIOS_GPIO_Off(Param[0]->Val->UnsignedCharacter);
				break;
			case 1:
				PIOS_GPIO_On(Param[0]->Val->UnsignedCharacter);
				break;
			case 2:
				PIOS_GPIO_Toggle(Param[0]->Val->UnsignedCharacter);
				break;
		}
	}
}

/* void int GPIORead(unsigned int): Reads the value of a general purpose output pin */
/*(unsigned int pin_num)*/
/*unsigned int pin_num: Number of the defined GPIO_IN pin from  target/board-info/pios_board.h; starting from 0*/
void SystemGPIORead(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	...
	{
		ReturnValue->Val->UnsignedCharacter = PIOS_GPIO_Read(Param[0]->Val->UnsignedInteger);
	}
}
#endif
...
#endif
#ifdef PIOS_INCLUDE_GPIO
	{ SystemGPIOWrite,		"void GPIOWrite(unsigned char, unsigned char);" },
	{ SystemGPIORead,		"unsigned char GPIORead(unsigned char);" },
#endif
	{ NULL, NULL }
};
