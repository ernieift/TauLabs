 ...
 * @author     Tau Labs, http://taulabs.org, Copyright (C) 2013-15
...
int32_t PIOS_PWM_Init(uintptr_t * pwm_id, const struct pios_pwm_cfg * cfg)
{
	...
	for (uint8_t i = 0; i < cfg->num_channels; i++) {
		...
		htim.Instance = chan->timer;
		TIM_ICInitStructure = cfg->tim_ic_init;
		HAL_TIM_IC_ConfigChannel(&htim, &TIM_ICInitStructure, chan->timer_chan);
		
		/* Enable the Capture Compare Interrupt Request */
		switch (chan->timer_chan) {
		case TIM_CHANNEL_1:
			__HAL_TIM_ENABLE_IT(&htim, TIM_IT_CC1 | TIM_IT_UPDATE);
			break;
		case TIM_CHANNEL_2:
			__HAL_TIM_ENABLE_IT(&htim, TIM_IT_CC2 | TIM_IT_UPDATE);
			break;
		case TIM_CHANNEL_3:
			__HAL_TIM_ENABLE_IT(&htim, TIM_IT_CC3 | TIM_IT_UPDATE);
			break;
		case TIM_CHANNEL_4:
...
static void PIOS_PWM_tim_edge_cb (uintptr_t tim_id, uintptr_t context, uint8_t chan_idx, uint16_t count)
{
	...
	TIM_HandleTypeDef htim = { .Instance = chan->timer, .Lock = HAL_UNLOCKED };
	TIM_IC_InitTypeDef TIM_ICInitStructure = pwm_dev->cfg->tim_ic_init;
	if (pwm_dev->CaptureState[chan_idx] == 0) {
		/* Switch states */
