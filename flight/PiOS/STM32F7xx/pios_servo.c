...
int32_t PIOS_Servo_Init(const struct pios_servo_cfg * cfg)
{
	...
	for (uint8_t i = 0; i < cfg->num_channels; i++) {
		...
		switch(chan->timer_chan) {
			case TIM_CHANNEL_1:
				TIM_OC1_SetConfig(chan->timer, (TIM_OC_InitTypeDef*)&cfg->tim_oc_init);
//				TIM_OC1PreloadConfig(chan->timer, TIM_OCPreload_Enable);
				break;
			case TIM_CHANNEL_2:
				TIM_OC2_SetConfig(chan->timer, (TIM_OC_InitTypeDef*)&cfg->tim_oc_init);
//				TIM_OC2PreloadConfig(chan->timer, TIM_OCPreload_Enable);
				break;
			case TIM_CHANNEL_3:
				TIM_OC3_SetConfig(chan->timer, (TIM_OC_InitTypeDef*)&cfg->tim_oc_init);
//				TIM_OC3PreloadConfig(chan->timer, TIM_OCPreload_Enable);
				break;
			case TIM_CHANNEL_4:
				TIM_OC4_SetConfig(chan->timer, (TIM_OC_InitTypeDef*)&cfg->tim_oc_init);
...
void PIOS_Servo_SetMode(const uint16_t * speeds, const enum pwm_mode *pwm_mode, uint8_t banks)
{
	...
	TIM_Base_InitTypeDef TIM_TimeBaseStructure = servo_cfg->tim_base_init;
	...
	for (uint8_t i = 0; (i < servo_cfg->num_channels) && (set < banks); i++) {
		...
		if (new) {
			...
			if (speeds[set] == 0) {
				...
				TIM_TimeBaseStructure.Period = (clk_rate / speeds[set]) - 1;
			}
			...
			if (chan->timer==TIM6 || chan->timer==TIM7) {
				...
				TIM_TimeBaseStructure.Prescaler = (PIOS_PERIPHERAL_APB2_CLOCK / clk_rate) - 1;
			} else {
				TIM_TimeBaseStructure.Prescaler = (PIOS_PERIPHERAL_APB1_CLOCK / clk_rate) - 1;
			}

			// Configure this timer appropriately.
			TIM_Base_SetConfig(chan->timer, &TIM_TimeBaseStructure);	
...
void PIOS_Servo_Set(uint8_t servo, float position)
{
	...
	switch(chan->timer_chan) {
		case TIM_CHANNEL_1:
			chan->timer->CCR1 = position;
			break;
		case TIM_CHANNEL_2:
			chan->timer->CCR2 = position;
			break;
		case TIM_CHANNEL_3:
			chan->timer->CCR3 = position;
			break;
		case TIM_CHANNEL_4:
			chan->timer->CCR4 = position;
			break;
	}
}
#else
...
void PIOS_Servo_Update()
{
	...
	for (uint8_t i = 0; i < servo_cfg->num_channels; i++) {
		...
		if (!(chan->timer->CR1 & TIM_CR1_CEN)) {
			/* enable it again and force an update */
			chan->timer->CR1 |= (TIM_CR1_CEN);
			chan->timer->EGR = TIM_EVENTSOURCE_UPDATE;
		}
	}
}
