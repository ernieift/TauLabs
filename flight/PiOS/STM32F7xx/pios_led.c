...
int32_t PIOS_LED_Init(const struct pios_led_cfg * cfg)
{
	...
	for (uint8_t i = 0; i < cfg->num_leds; i++) {
		...
		HAL_GPIO_Init(led->pin.gpio, (GPIO_InitTypeDef*)&led->pin.init);
		PIOS_LED_Off(i);
	}
...
void PIOS_LED_On(uint32_t led_id)
{
	...
	if (led->active_high)
		HAL_GPIO_WritePin(led->pin.gpio, led->pin.init.Pin, GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(led->pin.gpio, led->pin.init.Pin, GPIO_PIN_RESET);
}
...
void PIOS_LED_Toggle(uint32_t led_id)
{
	...
	HAL_GPIO_TogglePin(led->pin.gpio, led->pin.init.Pin);
}
