...
int32_t PIOS_TIM_InitClock(const struct pios_tim_clock_cfg * cfg)
{
	...
	htim.Instance = cfg->timer;
	htim.Init = cfg->time_base_init[0];
...
int32_t PIOS_TIM_InitChannels(uintptr_t * tim_id, const struct pios_tim_channel * channels, uint8_t num_channels, const struct pios_tim_callbacks * callbacks, uintptr_t context)
{
	...
	for (uint8_t i = 0; i < num_channels; i++) {
		...
		HAL_GPIO_Init(chan->pin.gpio, (GPIO_InitTypeDef*)&chan->pin.init);
	}
	...
	for (uint8_t i = 0; i < pios_tim_num_devs; i++) {
		...
		for (uint8_t j = 0; j < tim_dev->num_channels; j++) {
			...
			if (TIM_GetITStatus(timer, timer_it) == SET) {
				...
				switch(chan->timer_chan) {
				case TIM_CHANNEL_1:
					edge_count = timer->CCR1;
					break;
				case TIM_CHANNEL_2:
					edge_count = timer->CCR2;
					break;
				case TIM_CHANNEL_3:
					edge_count = timer->CCR3;
					break;
				case TIM_CHANNEL_4:
					edge_count = timer->CCR4;
					break;
				default:
					PIOS_Assert(0);
					break;
