...
uint32_t	PIOS_IAP_CheckRequest( void )
{
	...
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	reg1 = HAL_RTCEx_BKUPRead(&hrtc, MAGIC_REG_1 );
	reg2 = HAL_RTCEx_BKUPRead(&hrtc, MAGIC_REG_2 );
...
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	HAL_RTCEx_BKUPWrite(&hrtc, MAGIC_REG_1, IAP_MAGIC_WORD_1);
}

void	PIOS_IAP_SetRequest2(void)
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	HAL_RTCEx_BKUPWrite(&hrtc, MAGIC_REG_2, IAP_MAGIC_WORD_2);
}

void	PIOS_IAP_SetRequest3(void)
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	HAL_RTCEx_BKUPWrite(&hrtc, MAGIC_REG_2, IAP_MAGIC_WORD_3);
}

void	PIOS_IAP_ClearRequest(void)
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	HAL_RTCEx_BKUPWrite(&hrtc, MAGIC_REG_1, 0);
	HAL_RTCEx_BKUPWrite(&hrtc, MAGIC_REG_2, 0);
}

uint16_t PIOS_IAP_ReadBootCount(void)
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	return HAL_RTCEx_BKUPRead(&hrtc, IAP_BOOTCOUNT);
}

void PIOS_IAP_WriteBootCount (uint16_t boot_count)
{
	RTC_HandleTypeDef hrtc = { .Instance = RTC };
	HAL_RTCEx_BKUPWrite(&hrtc, IAP_BOOTCOUNT, boot_count);
}
